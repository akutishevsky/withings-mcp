...
- Concurrent requests for the same cold session share one in-flight rehydration (`rehydrating` Map) rather than racing
...
- `rotateToken()` is called by `tokenStore.rotateToken()` on the OAuth `refresh_token` grant, so sessions opened under the old bearer stay owned by the same client instead of being rejected as belonging to someone else
